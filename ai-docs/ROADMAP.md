...
- **Master Data — Equipment Catalog** — `EquipmentCatalogItem` + `/api/master-data/equipment` + `/master-data/equipment` UI; ~445 commercial OEM seed rows; project form snapshots into `Equipment`.
- **Master Data — Commodity + Country Catalogs** — `CommodityCatalogItem` / `CountryCatalogItem` + CRUD/ensure APIs + catalog UIs; new-project workflow composes engineering/fiscal defaults via `composeProjectDefaultsFromMasterData` (snapshot into `MiningProject`; no live FK). Reusable by future Decision Insights / Fleet / Benchmarking.
...
- **Decision Insights** — read-only executive interpretation layer (`lib/decision-insights.ts`, `/decision-insights`, `/api/projects/[id]/decision-insights`). Consumes existing NPV/IRR/payback/CAPEX/OPEX/cash-flow/sensitivity/Monte Carlo outputs; does not modify the economic engine.
