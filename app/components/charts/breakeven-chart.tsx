...
export function BreakevenChart({ basePrice, breakevenPrice, cashFlows, discountRate }: BreakevenChartProps) {
  ...
  const CustomTooltip = ({ active, payload }: {
    active?: boolean;
    payload?: Array<{ payload?: { price?: number; npv?: number } }>;
  }) => {
    if (!active || !payload || !payload.length) return null;
    const d = payload[0]?.payload;
    return (
      <div className="rounded-lg bg-background/95 border border-border p-3 shadow-xl text-sm">
        <p className="font-semibold mb-1">{t('chart.price')}: {d?.price} USD/ton</p>
        <p className={(d?.npv ?? 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}>
          NPV: {d?.npv?.toFixed(1)} MUSD
        </p>
      </div>
    );
  };
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <ComposedChart data={priceRange} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
          ...
          <XAxis dataKey="price" fontSize={11} label={{ value: t('chart.priceUsdTon'), position: 'insideBottomRight', offset: -5, fontSize: 11 }} />
          <YAxis fontSize={11} tickFormatter={(v: number) => `${v.toFixed(0)}`} label={{ value: t('chart.npvMusd'), angle: -90, position: 'insideLeft', fontSize: 11 }} />
          <Tooltip content={<CustomTooltip />} />
          <ReferenceLine y={0} stroke="hsl(var(--foreground))" strokeDasharray="5 5" strokeOpacity={0.5} label={{ value: t('chart.npvZero'), position: 'right', fontSize: 10 }} />
